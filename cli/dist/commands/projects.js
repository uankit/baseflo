...
export async function projectsCreateCommand(slug, opts) {
    ...
    const result = await client.projects.create({
        ...
        ...(opts.deploymentMode !== undefined
            ? {
                deployment_mode: opts.deploymentMode,
            }
            : {}),
    });
    ...
    if (creds) {
        const next = { ...creds, activeProjectId: result.project_id };
        saveCredentials(next);
        console.log(colors.dim("  set as active project for `ask` / `refine` / `export`"));
    }
}
export async function projectsGetCommand(id) {
    const client = buildClient({ requireAuth: true });
    try {
        ...
        console.log(colors.dim(`  current_version_id       ${project.current_version_id ?? "—"}`));
        console.log(colors.dim(`  tenant_data_schema_name  ${project.tenant_data_schema_name ?? "—"}`));
