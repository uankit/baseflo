...
export async function refineCommand(request, opts) {
    const projectId = activeProjectIdOrThrow(opts.projectId);
    const client = buildClient({ requireAuth: true });
...
function formatEventSummary(eventName, data) {
    ...
    if (eventName === "workspace.ready") {
        ...
        const versionId = typeof data["project_version_id"] === "string"
    ...
    if (keys.length === 0)
        return "";
    if (keys.length === 1)
        return `${keys[0]}=${String(data[keys[0]])}`;
