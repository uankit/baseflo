...
export async function devCommand(opts) {
    ...
    console.log(colors.dim(`  Starting Postgres + Redis from ${composeFile}…`));
    await runProcess("docker", ["compose", "-f", composeFile, "up", "-d"]);
    console.log(colors.green("✓") + " Postgres + Redis running.");
    console.log("");
    console.log(colors.bold("Next steps:"));
    console.log(`  1. Apply migrations:    ${colors.cyan("cd server && uv run alembic upgrade head")}`);
    console.log(`  2. Start the API:       ${colors.cyan("cd server && uv run uvicorn app.main:app --reload")}`);
    console.log(`  3. Sign in:             ${colors.cyan("baseflo login")}`);
    console.log("");
