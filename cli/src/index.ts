...
import {
  connectShopifyCommand,
  connectStripeCommand,
  connectSheetsCommand,
...
program
  ...
  .option(
    "--base-url <url>",
    "API base URL (default: $BASEFLO_API_BASE_URL or http://localhost:8000).",
  )
  .action(async (opts: { email?: string; baseUrl?: string }) => {
...
program
  ...
  .option(
    ...
    "hosted | byo_db | self_host | local_dev (default: hosted).",
  )
  .action(
    async (
      slug: string,
...
connect
  ...
  .action(async (opts: { shopDomain: string; projectId?: string }) => {
...
connect
  ...
  .requiredOption(
    "--api-key <key>",
    "Restricted or secret Stripe API key (sk_... or rk_...).",
  )
  .option("--project-id <uuid>", "Override the active project.")
  .action(async (opts: { apiKey: string; projectId?: string }) => {
...
connect
  ...
  .option("--project-id <uuid>", "Override the active project.")
  .action(async (opts: { spreadsheetId: string; projectId?: string }) => {
...
program
  ...
  .option("--project-id <uuid>", "Override the active project.")
  .action(async (prompt: string, opts: { projectId?: string }) => {
...
program
  ...
  .option("--project-id <uuid>", "Override the active project.")
  .action(async (request: string, opts: { projectId?: string }) => {
...
program
  ...
  .action(async (projectIdArg: string | undefined, opts: { out?: string }) => {
...
program
  ...
  .action(async (opts: { composeFile?: string; detached?: boolean }) => {
...
async function runWithErrorHandler(fn: () => Promise<void>): Promise<void> {
  try {
    await fn();
  } catch (err) {
