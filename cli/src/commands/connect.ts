...
export async function connectShopifyCommand(opts: {
  ...
  console.log(
    colors.dim(
      "  After granting access, Shopify redirects to /api/v1/oauth/shopify/callback. " +
...
export async function connectSheetsCommand(opts: {
  ...
  console.log(
    colors.dim(
      "  After granting `spreadsheets` + `drive.metadata.readonly`, Google " +
...
export async function connectorsListCommand(opts: {
  ...
  for (const c of list.connectors) {
    console.log(
      `${colors.bold(c.kind.padEnd(15))} ${colors.dim(c.id)} ${colors.cyan(c.display_name)} ` +
