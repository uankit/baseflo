...
export async function loginCommand(opts: LoginOpts): Promise<void> {
  const baseUrl =
    opts.baseUrl ?? process.env.BASEFLO_API_BASE_URL ?? "http://localhost:8000";
  ...
  try {
    ...
    console.log(
      colors.dim(
        "  In dev (LogEmailSender), the link is in the server's stdout. " +
    ...
    if (!sessionToken) {
      throw new Error(
        "Verify endpoint did not return a session cookie. " +
          "Check that the server's auth middleware is installed.",
      );
    }

    const creds: Credentials = { baseUrl, sessionToken, email };
...
export async function whoamiCommand(): Promise<void> {
  ...
  try {
    ...
    console.log(
      colors.bold("organization") + `   ${me.organization_id} (role=${me.role ?? "—"})`,
    );
...
function parseSessionTokenFromCookie(setCookie: string): string | null {
